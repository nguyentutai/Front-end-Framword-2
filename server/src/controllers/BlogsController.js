...
class BlogsController {
  ...
  async getBlogSlug(req, res) {
    try {
      ...
      if (data) {
        return res.status(200).send({
          message: "Hiển thị bài viết theo slug thành công!",
          data,
        });
      }
    } catch (error) {
  ...
  async addBlog(req, res) {
    try {
      ...
      const dataUser = await userSchema.findByIdAndUpdate(
        data.userId,
        {
          $push: {
            blogId: data._id,
  ...
  async updateBlog(req, res) {
    try {
      const data = await blogShema.findByIdAndUpdate(req.params.id, req.body, {
        new: true,
      });
      if (data) {
        return res.status(200).send({
  ...
  async updateStatusBlog(req, res) {
    try {
      ...
      const data = await blogShema.findByIdAndUpdate(
        req.params.id,
        { status },
        { new: true }
      );
